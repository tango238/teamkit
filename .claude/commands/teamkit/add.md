...
- **ユーザーへの質問**: テキストメッセージで質問を表示し、ユーザーの次のメッセージ入力を待つ（`AskUserQuestion` ツールは使用しない）
...
- `{{specPath}}` が既に存在するか確認する
  - 存在する場合は警告を表示: 「`{{specPath}}` は既に存在します。上書きしますか？」とテキストで確認し、ユーザーの回答を待つ
    - 「はい」の場合 → 続行
    - 「いいえ」の場合 → **STOP**
...
以下の情報をユーザーから **1つずつ順番に** 聞き出す。

**重要なルール:**
- 質問は **1つずつ** 行い、ユーザーの回答を **必ず待ってから** 次の質問に進むこと
- **複数の質問を同時に行ってはいけない**
- ユーザーの回答が **空または未入力** の場合は、同じ質問を再度行うこと（スキップしない）
- 各質問はテキストメッセージとして表示し、ユーザーが通常のチャット入力で回答するのを待つこと
- `AskUserQuestion` ツールは使用しない。代わりに質問をテキストで表示して、ユーザーの次のメッセージを待つこと

### 2.1 背景
以下のメッセージを表示して、ユーザーの回答を待つ:
```
「{{featureName}}」が必要になった背景を教えてください。
（どんな課題や状況があるか）
```
→ ユーザーの回答を受け取ったら 2.2 へ進む
...
（誰が何をして、どうなるか）
```
→ ユーザーの回答を受け取ったら 2.5 へ進む

### 2.5 要件
各アクター（2.3で回答されたもの）について、以下のメッセージを表示して回答を待つ:
```
「{{アクター名}}」がこの機能でできることを教えてください。
（例：登録、一覧表示、編集、削除、承認など）
```
→ すべてのアクターについて回答を受け取ったら Step 3 へ進む
...
- ユーザーに確認:「この内容でREADME.mdを作成してよろしいですか？」とテキストで表示し、ユーザーの回答を待つ
  - 「はい」の場合 → Step 4へ進む
